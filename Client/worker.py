...
def determine_scale_factor(config, dev_name, base_name):
    ...
    if test['scale_method'] == 'DEV':
        factor = test['scale_nps'] / dev_nps
        print ('\nScale Factor (Using Dev): %.4f' % (factor))
        return {'dev': factor, 'base': factor}

    elif test['scale_method'] == 'BASE':
        factor = test['scale_nps'] / base_nps
        print ('\nScale Factor (Using Base): %.4f' % (factor))
        return {'dev': factor, 'base': factor}

    else:
        dev_expected = test.get('dev_scale_nps') or test['scale_nps']
        base_expected = test.get('base_scale_nps') or test['scale_nps']

        dev_factor = dev_expected / dev_nps
        base_factor = base_expected / base_nps

        print ('\nScale Factor (Using Dev): %.4f' % (dev_factor))
        print ('\nScale Factor (Using Base): %.4f' % (base_factor))

        return {'dev': dev_factor, 'base': base_factor}
...
def complete_workload(config):
    ...
    scale_factors = determine_scale_factor(config, dev_name, base_name)
    ...
    with ThreadPoolExecutor(max_workers=runner_cnt) as executor:
        ...
        for x in range(runner_cnt):
            cmd = build_runner_command(config, dev_name, base_name, scale_factors, timestamp, x)
        ...
        if config.workload['test']['upload_pgns'] != 'FALSE':
            ...
            ServerReporter.report_pgn(config, pgn_util.compress_pgn_files(pgn_files, scale_factors, compact))
...
def build_runner_command(config, dev_cmd, base_cmd, scale_factors, timestamp, runner_idx):
    ...
    flags += ' ' + MatchRunner.engine_settings(config, dev_cmd, 'dev', scale_factors['dev'], runner_idx)
    flags += ' ' + MatchRunner.engine_settings(config, base_cmd, 'base', scale_factors['base'], runner_idx)
