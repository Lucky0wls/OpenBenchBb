...
def format_scale_factor(scale_factors):
    if isinstance(scale_factors, dict):
        return 'dev=%.4f base=%.4f' % (scale_factors['dev'], scale_factors['base'])
    return str(scale_factors)

def process_pgn_file(file_name, scale_factors, compact):

    output = []

    for headers, move_text in pgn_iterator(file_name):
        headers['ScaleFactor'] = format_scale_factor(scale_factors)
...
def compress_pgn_files(file_names, scale_factors, compact):

    compressed_text = []
    for fname in file_names:
        print ('Compressing %s...' % fname)
        compressed_text.append(process_pgn_file(fname, scale_factors, compact))
...
if __name__ == '__main__':
    ...
    result = process_pgn_file(args.pgn, scale_factors=1.0, compact=args.compact)
    print (result)
