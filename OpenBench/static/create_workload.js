...
window.update_scale_fields = function update_scale_fields() {
    const method = document.getElementById('scale_method').value;

    const single = method != 'BOTH';
    document.getElementById('scale_nps_row').style.display = single ? '' : 'none';
    document.getElementById('dev_scale_nps_row').style.display = single ? 'none' : '';
    document.getElementById('base_scale_nps_row').style.display = single ? 'none' : '';
}
...
function change_engine(engine, target, workload_type) {
    ...
    set_option('dev_scale_nps', config.engines[get_dev_engine()].nps);
    set_option('base_scale_nps', config.engines[get_base_engine()].nps);
    set_option('scale_method', workload_type == 'TUNE' ? 'DEV' : 'BASE');
    window.update_scale_fields();
...
document.addEventListener('DOMContentLoaded', window.update_scale_fields);