...
def assign_scaling_fields(test, request):
    test.scale_method = request.POST['scale_method']

    if test.scale_method == 'BOTH':
        test.dev_scale_nps = int(request.POST['dev_scale_nps'])
        test.base_scale_nps = int(request.POST['base_scale_nps'])
        test.scale_nps = (test.dev_scale_nps + test.base_scale_nps) // 2
    else:
        test.scale_nps = int(request.POST['scale_nps'])
        test.dev_scale_nps = test.scale_nps
        test.base_scale_nps = test.scale_nps
...
def create_new_test(request):
    ...
    assign_scaling_fields(test, request)
