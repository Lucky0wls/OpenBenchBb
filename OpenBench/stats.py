...
def nElo(results):
    ...
    scale = 800 / math.log(10)
    denom = math.sqrt((2 if len(results) == 5 else 1) * var)
    nelo = (mu - 0.5) / denom * scale

    var_per_sample = var / N
    mu_min = mu + scipy.stats.norm.ppf(0.025) * math.sqrt(var_per_sample)
    mu_max = mu + scipy.stats.norm.ppf(0.975) * math.sqrt(var_per_sample)

    nelo_min = (mu_min - 0.5) / denom * scale
    nelo_max = (mu_max - 0.5) / denom * scale

    return nelo, max(nelo_max - nelo, nelo - nelo_min)
